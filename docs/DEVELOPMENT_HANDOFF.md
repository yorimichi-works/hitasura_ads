...
広告ゲーム中のBGM 8曲（8bit27、8bit28、ネオロック82、ピアノ04、ピアノ17、サイバー41、サイバー44、サイバー45）は魔王魂（https://maou.audio/、作曲：森田交一）の公式音源です。151広告ごとの意味プロファイルに従い、高揚・レトロ・リラックス・シリアス・間抜け・パズル・不穏・サイバーのムードから選曲します。待機・図鑑・プロフィール画面では再生せず、広告ゲームのオーバーレイ表示中だけ再生します。商用利用可・著作表記必須のライセンスのため、設定画面に「音楽：魔王魂」のクレジットを表示しています。詳細は`THIRD_PARTY_NOTICES.md`を参照してください。
