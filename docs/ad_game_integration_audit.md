# No.1-151 ゲーム画面統合一覧

現行コードと実ファイルを正本として、採用画像、操作対象、コード描画の責務を確定した一覧。

|No.|タイトル|ゲームタイプ|採用前景画像|採用背景|コード描画として残したもの|画像化したゲームオブジェクト|操作方法|評価|動作確認|
|---:|---|---|---|---|---|---|---|:---:|---|
|001|あなたは本日999,999人目！|tapChallenge|なし|sheet4_01.png|HitArea・進捗|画像不要または専用素材なし。意味一致するコード描画|画像/対象を3回タップ|B|151件共通Widgetテスト対象|
|002|今すぐクリックしないでください！|tapChallenge|なし|sheet4_02.png|HitArea・進捗|画像不要または専用素材なし。意味一致するコード描画|画像/対象を3回タップ|B|151件共通Widgetテスト対象|
|003|この矢印の先に未来がある|tapChallenge|なし|sheet5_15.png|HitArea・進捗|画像不要または専用素材なし。意味一致するコード描画|画像/対象を3回タップ|B|151件共通Widgetテスト対象|
|004|残り時間、永遠の10秒|countdownStop|sheet1_12.png|sheet4_01.png|数字・タイマー|sheet1_12.png|残り1でタップ|A|151件共通Widgetテスト対象|
|005|驚異の1円OFF！|tapChallenge|sheet1_16.png|sheet4_02.png|HitArea・進捗|sheet1_16.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|006|世界が震えた0.5％OFF|tapChallenge|sheet1_16.png|sheet5_15.png|HitArea・進捗|sheet1_16.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|007|閉じるボタンを探せ！|tapChallenge|sheet1_14.png|sheet4_01.png|HitArea・進捗|sheet1_14.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|008|ダウンロード率120％|tapChallenge|sheet1_09.png|sheet4_02.png|HitArea・進捗|sheet1_09.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|009|NEW！たぶんNEW！|tapChallenge|sheet2_19.png|sheet5_15.png|HitArea・進捗|sheet2_19.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|010|工事中なのに営業中|tapChallenge|なし|sheet4_01.png|HitArea・進捗|画像不要または専用素材なし。意味一致するコード描画|画像/対象を3回タップ|B|151件共通Widgetテスト対象|
|011|水が入る！奇跡のコップ|tapChallenge|sheet2_12.png|sheet4_18.png|HitArea・進捗|sheet2_12.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|012|座れる椅子、ついに登場|tapChallenge|sheet2_10.png|sheet5_11.png|HitArea・進捗|sheet2_10.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|013|絶対に開かない傘|tapChallenge|sheet2_13.png|sheet5_19.png|HitArea・進捗|sheet2_13.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|014|何も入らない財布|tapChallenge|sheet2_11.png|sheet4_18.png|HitArea・進捗|sheet2_11.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|015|寝るためだけの枕|tapChallenge|sheet2_07.png|sheet5_11.png|HitArea・進捗|sheet2_07.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|016|切れる包丁・改|tapChallenge|sheet2_12.png|sheet5_19.png|HitArea・進捗|sheet2_12.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|017|冷たい冷蔵庫|tapChallenge|sheet2_10.png|sheet4_18.png|HitArea・進捗|sheet2_10.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|018|歩ける靴 PREMIUM|tapChallenge|sheet2_13.png|sheet5_11.png|HitArea・進捗|sheet2_13.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|019|持てるカバン PRO MAX|tapChallenge|sheet2_11.png|sheet5_19.png|HitArea・進捗|sheet2_11.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|020|飲める水 2026|tapChallenge|sheet2_07.png|sheet4_18.png|HitArea・進捗|sheet2_07.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|021|専門家100人中1人が推薦|choice|sheet1_10.png|sheet5_04.png|選択枠・正誤判定|sheet1_10.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|022|堂々の第1位（1商品中）|choice|sheet2_16.png|sheet4_07.png|選択枠・正誤判定|sheet2_16.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|023|★★★★★ 0件のレビュー|choice|sheet2_05.png|sheet5_08.png|選択枠・正誤判定|sheet2_05.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|024|満足度101％|choice|sheet1_17.png|sheet5_04.png|選択枠・正誤判定|sheet1_17.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|025|リピート率300％|choice|sheet1_10.png|sheet4_07.png|選択枠・正誤判定|sheet1_10.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|026|利用者の99％が人間|choice|sheet2_16.png|sheet5_08.png|選択枠・正誤判定|sheet2_16.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|027|広告博士が認めた広告|choice|sheet2_05.png|sheet5_04.png|選択枠・正誤判定|sheet2_05.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|028|今年最も広告だった広告|choice|sheet1_17.png|sheet4_07.png|選択枠・正誤判定|sheet1_17.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|029|比較したら勝ってました|choice|sheet1_10.png|sheet5_08.png|選択枠・正誤判定|sheet1_10.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|030|口コミで話題！（口コミ1件）|choice|sheet2_16.png|sheet5_04.png|選択枠・正誤判定|sheet2_16.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|031|王様を助けろ！初級編|pinPull|sheet1_02.png|palace_treasure_hall.jpg|ピン・液体・順序判定|sheet1_02.png|ピンを順番にタップ|A|151件共通Widgetテスト対象|
|032|王様、炎上する|pinPull|sheet1_02.png|palace_treasure_hall.jpg|ピン・液体・順序判定|sheet1_02.png|ピンを順番にタップ|A|151件共通Widgetテスト対象|
|033|王様と三本のピン|pinPull|sheet1_02.png|palace_treasure_hall.jpg|ピン・液体・順序判定|sheet1_02.png|ピンを順番にタップ|A|151件共通Widgetテスト対象|
|034|宝より王を選べ|pinPull|sheet1_02.png|palace_treasure_hall.jpg|ピン・液体・順序判定|sheet1_02.png|ピンを順番にタップ|A|151件共通Widgetテスト対象|
|035|王様、また溺れる|pinPull|sheet1_02.png|palace_treasure_hall.jpg|ピン・液体・順序判定|sheet1_02.png|ピンを順番にタップ|A|151件共通Widgetテスト対象|
|036|溶岩 VS 王様|pinPull|sheet1_02.png|palace_treasure_hall.jpg|ピン・液体・順序判定|sheet1_02.png|ピンを順番にタップ|A|151件共通Widgetテスト対象|
|037|王様、また燃える|pinPull|sheet1_02.png|palace_treasure_hall.jpg|ピン・液体・順序判定|sheet1_02.png|ピンを順番にタップ|A|151件共通Widgetテスト対象|
|038|王様救出率2％|pinPull|sheet1_02.png|palace_treasure_hall.jpg|ピン・液体・順序判定|sheet1_02.png|ピンを順番にタップ|A|151件共通Widgetテスト対象|
|039|王様はそこじゃない|pinPull|sheet1_02.png|palace_treasure_hall.jpg|ピン・液体・順序判定|sheet1_02.png|ピンを順番にタップ|A|151件共通Widgetテスト対象|
|040|王様と謎の魚|pinPull|sheet1_02.png|palace_treasure_hall.jpg|ピン・液体・順序判定|sheet1_02.png|ピンを順番にタップ|A|151件共通Widgetテスト対象|
|041|王様、大富豪になる|pinPull|sheet1_02.png|stone_dungeon.jpg|ピン・液体・順序判定|sheet1_02.png|ピンを順番にタップ|A|151件共通Widgetテスト対象|
|042|王様、地下100階へ|pinPull|sheet1_02.png|stone_dungeon.jpg|ピン・液体・順序判定|sheet1_02.png|ピンを順番にタップ|A|151件共通Widgetテスト対象|
|043|王様と絶対抜くなピン|pinPull|sheet1_02.png|stone_dungeon.jpg|ピン・液体・順序判定|sheet1_02.png|ピンを順番にタップ|A|151件共通Widgetテスト対象|
|044|王様の休日|pinPull|sheet1_02.png|stone_dungeon.jpg|ピン・液体・順序判定|sheet1_02.png|ピンを順番にタップ|A|151件共通Widgetテスト対象|
|045|王様FINALっぽい何か|pinPull|sheet1_02.png|stone_dungeon.jpg|ピン・液体・順序判定|sheet1_02.png|ピンを順番にタップ|A|151件共通Widgetテスト対象|
|046|＋10か×2か、それが問題だ|numberGate|sheet2_01.png|sheet5_06.png|ゲート・倍率・計算|sheet2_01.png|倍率ゲートをタップ|A|151件共通Widgetテスト対象|
|047|棒人間、増えすぎる|numberGate|sheet2_03.png|sheet5_10.png|ゲート・倍率・計算|sheet2_03.png|倍率ゲートをタップ|A|151件共通Widgetテスト対象|
|048|1人から999999人へ|numberGate|sheet2_05.png|sheet4_03.png|ゲート・倍率・計算|sheet2_05.png|倍率ゲートをタップ|A|151件共通Widgetテスト対象|
|049|×100を選んだだけなのに|numberGate|sheet2_16.png|sheet5_06.png|ゲート・倍率・計算|sheet2_16.png|倍率ゲートをタップ|A|151件共通Widgetテスト対象|
|050|＋5の方が強かった|numberGate|sheet1_09.png|sheet5_10.png|ゲート・倍率・計算|sheet1_09.png|倍率ゲートをタップ|A|151件共通Widgetテスト対象|
|051|レベル1から宇宙王へ|numberGate|sheet2_01.png|sheet4_03.png|ゲート・倍率・計算|sheet2_01.png|倍率ゲートをタップ|A|151件共通Widgetテスト対象|
|052|LV.9999なのに弱い|numberGate|sheet2_03.png|sheet5_06.png|ゲート・倍率・計算|sheet2_03.png|倍率ゲートをタップ|A|151件共通Widgetテスト対象|
|053|敵が急に巨大化しました|numberGate|sheet2_05.png|sheet5_10.png|ゲート・倍率・計算|sheet2_05.png|倍率ゲートをタップ|A|151件共通Widgetテスト対象|
|054|剣を拾ったら社長になった|numberGate|sheet2_16.png|sheet4_03.png|ゲート・倍率・計算|sheet2_16.png|倍率ゲートをタップ|A|151件共通Widgetテスト対象|
|055|棒人間帝国の終焉|numberGate|sheet1_09.png|sheet5_06.png|ゲート・倍率・計算|sheet1_09.png|倍率ゲートをタップ|A|151件共通Widgetテスト対象|
|056|ネジを外すだけだったのに|dragSort|sheet1_03.png|sheet5_05.png|DragTarget・当たり判定|sheet1_03.png|画像そのものをドラッグ&ドロップ|A|151件共通Widgetテスト対象|
|057|そのネジ、そこじゃない|dragSort|sheet1_03.png|sheet4_08.png|DragTarget・当たり判定|sheet1_03.png|画像そのものをドラッグ&ドロップ|A|151件共通Widgetテスト対象|
|058|水をコップへ導け！|drawPath|なし|sheet4_06.png|線・経路判定|画像不要または専用素材なし。意味一致するコード描画|画像間をドラッグして線描画|B|151件共通Widgetテスト対象|
|059|蜂から犬っぽい何かを守れ|drawPath|rescue_dog.png|sunny_grassland.jpg|線・経路判定|rescue_dog.png|画像間をドラッグして線描画|A|151件共通Widgetテスト対象|
|060|線を引けば全部解決|drawPath|sheet1_14.png|sheet4_08.png|線・経路判定|sheet1_14.png|画像間をドラッグして線描画|A|151件共通Widgetテスト対象|
|061|駐車場から出たいだけ|dragSort|なし|parking_lot.jpg|DragTarget・当たり判定|画像不要または専用素材なし。意味一致するコード描画|画像そのものをドラッグ&ドロップ|B|151件共通Widgetテスト対象|
|062|赤い車だけ特別扱い|dragSort|なし|parking_lot.jpg|DragTarget・当たり判定|画像不要または専用素材なし。意味一致するコード描画|画像そのものをドラッグ&ドロップ|B|151件共通Widgetテスト対象|
|063|色を分けろ！何のために？|dragSort|sheet2_18.png|sheet4_08.png|DragTarget・当たり判定|sheet2_18.png|画像そのものをドラッグ&ドロップ|A|151件共通Widgetテスト対象|
|064|ボールを正しい穴へ|dragSort|sheet2_14.png|sheet4_06.png|DragTarget・当たり判定|sheet2_14.png|画像そのものをドラッグ&ドロップ|A|151件共通Widgetテスト対象|
|065|3秒で解けるIQ999パズル|dragSort|sheet1_14.png|sheet5_05.png|DragTarget・当たり判定|sheet1_14.png|画像そのものをドラッグ&ドロップ|A|151件共通Widgetテスト対象|
|066|汚れすぎた靴|choice|sheet2_04.png|dirty_room.jpg|選択枠・正誤判定|sheet2_04.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|067|この部屋をなんとかして！|choice|sheet2_02.png|renovated_room.jpg|選択枠・正誤判定|sheet2_02.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|068|掃除したら豪邸になった|choice|sheet2_12.png|dirty_room.jpg|選択枠・正誤判定|sheet2_12.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|069|貧乏から億万長者まで15秒|choice|sheet2_18.png|renovated_room.jpg|選択枠・正誤判定|sheet2_18.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|070|所持金3円から始めます|choice|sheet2_19.png|dirty_room.jpg|選択枠・正誤判定|sheet2_19.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|071|選ぶだけで人生逆転!?|choice|sheet2_04.png|renovated_room.jpg|選択枠・正誤判定|sheet2_04.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|072|服を着替えたら王族になった|choice|sheet2_02.png|dirty_room.jpg|選択枠・正誤判定|sheet2_02.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|073|ボロ家から宮殿へ|choice|sheet2_12.png|renovated_room.jpg|選択枠・正誤判定|sheet2_12.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|074|掃除力9999|choice|sheet2_18.png|dirty_room.jpg|選択枠・正誤判定|sheet2_18.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|075|AFTERが別人|choice|sheet2_19.png|renovated_room.jpg|選択枠・正誤判定|sheet2_19.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|076|小魚から海の王へ|tapChallenge|sheet2_14.png|sheet5_03.png|HitArea・進捗|sheet2_14.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|077|魚が魚を超えた日|tapChallenge|sheet2_15.png|sheet5_06.png|HitArea・進捗|sheet2_15.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|078|リンゴ＋リンゴ＝なぜかメロン|tapChallenge|sheet2_09.png|sheet4_10.png|HitArea・進捗|sheet2_09.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|079|メロン＋メロン＝冷蔵庫|tapChallenge|sheet2_18.png|sheet5_03.png|HitArea・進捗|sheet2_18.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|080|何でも合体させればいい|tapChallenge|sheet2_14.png|sheet5_06.png|HitArea・進捗|sheet2_14.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|081|LV.1スライムっぽくない何か|tapChallenge|sheet2_15.png|sheet4_10.png|HitArea・進捗|sheet2_15.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|082|育てたら巨大になりすぎた|tapChallenge|sheet2_09.png|sheet5_03.png|HitArea・進捗|sheet2_09.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|083|ペット育成 8秒目|tapChallenge|sheet2_18.png|sheet5_06.png|HitArea・進捗|sheet2_18.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|084|卵から社長が生まれた|tapChallenge|sheet2_14.png|sheet4_10.png|HitArea・進捗|sheet2_14.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|085|進化先：広告|tapChallenge|sheet2_15.png|sheet5_03.png|HitArea・進捗|sheet2_15.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|086|777っぽいもの|timing|sheet1_18.png|sheet4_07.png|ゲージ・針・成功帯|sheet1_18.png|成功帯で停止|A|151件共通Widgetテスト対象|
|087|超激熱っぽい！|timing|sheet1_18.png|sheet4_02.png|ゲージ・針・成功帯|sheet1_18.png|成功帯で停止|A|151件共通Widgetテスト対象|
|088|大当たり!? たぶん！|timing|sheet1_18.png|sheet5_08.png|ゲージ・針・成功帯|sheet1_18.png|成功帯で停止|A|151件共通Widgetテスト対象|
|089|コイン＋999999|timing|sheet1_18.png|sheet4_07.png|ゲージ・針・成功帯|sheet1_18.png|成功帯で停止|A|151件共通Widgetテスト対象|
|090|回せ！無料スロット風広告|timing|sheet1_18.png|sheet4_02.png|ゲージ・針・成功帯|sheet1_18.png|成功帯で停止|A|151件共通Widgetテスト対象|
|091|全部当たりに見えるルーレット|timing|sheet1_18.png|sheet5_08.png|ゲージ・針・成功帯|sheet1_18.png|成功帯で停止|A|151件共通Widgetテスト対象|
|092|スクラッチしたら広告だった|scratch|sheet2_19.png|sheet4_07.png|スクラッチ面・削除判定|sheet2_19.png|面を指でこする|A|151件共通Widgetテスト対象|
|093|超絶プレミアム広告パック|packOpen|sheet2_14.png|delivery_warehouse.jpg|スワイプ距離・開封枠|sheet2_14.png|画像付きパックを上へスワイプ|A|151件共通Widgetテスト対象|
|094|SSR大量封入っぽいパック|packOpen|sheet2_15.png|delivery_warehouse.jpg|スワイプ距離・開封枠|sheet2_15.png|画像付きパックを上へスワイプ|A|151件共通Widgetテスト対象|
|095|残り3口！（増えます）|packOpen|sheet2_11.png|delivery_warehouse.jpg|スワイプ距離・開封枠|sheet2_11.png|画像付きパックを上へスワイプ|A|151件共通Widgetテスト対象|
|096|市場価格999999円相当!?|packOpen|sheet1_07.png|delivery_warehouse.jpg|スワイプ距離・開封枠|sheet1_07.png|画像付きパックを上へスワイプ|A|151件共通Widgetテスト対象|
|097|幻の広告カード開封|packOpen|sheet2_14.png|delivery_warehouse.jpg|スワイプ距離・開封枠|sheet2_14.png|画像付きパックを上へスワイプ|A|151件共通Widgetテスト対象|
|098|ラストワンじゃない賞|packOpen|sheet2_15.png|delivery_warehouse.jpg|スワイプ距離・開封枠|sheet2_15.png|画像付きパックを上へスワイプ|A|151件共通Widgetテスト対象|
|099|UR『すごい水』|packOpen|sheet2_11.png|delivery_warehouse.jpg|スワイプ距離・開封枠|sheet2_11.png|画像付きパックを上へスワイプ|A|151件共通Widgetテスト対象|
|100|100番記念・何も当たらない祭|packOpen|sheet1_07.png|delivery_warehouse.jpg|スワイプ距離・開封枠|sheet1_07.png|画像付きパックを上へスワイプ|A|151件共通Widgetテスト対象|
|101|AIがあなたの年齢を当てます|choice|sheet2_07.png|sheet4_08.png|選択枠・正誤判定|sheet2_07.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|102|AIが考えたAI広告|choice|sheet2_06.png|sheet4_06.png|選択枠・正誤判定|sheet2_06.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|103|あなたの広告タイプ診断|choice|sheet2_02.png|sheet5_02.png|選択枠・正誤判定|sheet2_02.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|104|3秒で分かる性格診断|choice|sheet2_05.png|sheet4_08.png|選択枠・正誤判定|sheet2_05.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|105|IQ999の人だけ解けます|choice|sheet2_07.png|sheet4_06.png|選択枠・正誤判定|sheet2_07.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|106|あなたの隠れた才能は？|choice|sheet2_06.png|sheet5_02.png|選択枠・正誤判定|sheet2_06.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|107|AIが選ぶ今日の商品|choice|sheet2_02.png|sheet4_08.png|選択枠・正誤判定|sheet2_02.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|108|未来の顔を予測しました|choice|sheet2_05.png|sheet4_06.png|選択枠・正誤判定|sheet2_05.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|109|あなたに最適な広告です|choice|sheet2_07.png|sheet5_02.png|選択枠・正誤判定|sheet2_07.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|110|AI搭載ボタン|tapChallenge|sheet2_06.png|sheet4_08.png|HitArea・進捗|sheet2_06.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|111|知らないと損！→知っても得しない|tapChallenge|sheet2_06.png|sheet4_17.png|HitArea・進捗|sheet2_06.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|112|衝撃の事実はCMのあと|tapChallenge|sheet2_02.png|sheet5_16.png|HitArea・進捗|sheet2_02.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|113|この動画を最後まで見てください|tapChallenge|sheet1_17.png|sheet4_16.png|HitArea・進捗|sheet1_17.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|114|コメント欄が全員絶賛|tapChallenge|sheet2_05.png|sheet4_17.png|HitArea・進捗|sheet2_05.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|115|人生変わりました！（何が？）|tapChallenge|sheet2_06.png|sheet5_16.png|HitArea・進捗|sheet2_06.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|116|たった3日でこうなった|tapChallenge|sheet2_02.png|sheet4_16.png|HitArea・進捗|sheet2_02.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|117|正直、教えたくありません|tapChallenge|sheet1_17.png|sheet4_17.png|HitArea・進捗|sheet1_17.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|118|友達には秘密にしてください|tapChallenge|sheet2_05.png|sheet5_16.png|HitArea・進捗|sheet2_05.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|119|今、若者の間で話題！|tapChallenge|sheet2_06.png|sheet4_16.png|HitArea・進捗|sheet2_06.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|120|1000万再生っぽい動画|tapChallenge|sheet2_02.png|sheet4_17.png|HitArea・進捗|sheet2_02.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|121|警告：広告が不足しています|tapChallenge|sheet1_19.png|sheet5_09.png|HitArea・進捗|sheet1_19.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|122|あなたの広告は古くなっています|tapChallenge|sheet1_17.png|sheet5_01.png|HitArea・進捗|sheet1_17.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|123|残り1個！から増える在庫|tapChallenge|sheet1_12.png|delivery_warehouse.jpg|HitArea・進捗|sheet1_12.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|124|今だけ！毎日開催|tapChallenge|sheet1_13.png|sheet5_09.png|HitArea・進捗|sheet1_13.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|125|本日最終日・第438日目|tapChallenge|sheet1_15.png|sheet5_01.png|HitArea・進捗|sheet1_15.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|126|あと5秒で終了します|tapChallenge|sheet1_19.png|sheet4_14.png|HitArea・進捗|sheet1_19.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|127|99％で止まるアップデート|tapChallenge|sheet1_17.png|sheet5_09.png|HitArea・進捗|sheet1_17.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|128|容量が足りません（広告の）|tapChallenge|sheet1_12.png|sheet5_01.png|HitArea・進捗|sheet1_12.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|129|この広告を見逃すと損!?|tapChallenge|sheet1_13.png|sheet4_14.png|HitArea・進捗|sheet1_13.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|130|緊急でも何でもない速報|tapChallenge|sheet1_15.png|sheet5_09.png|HitArea・進捗|sheet1_15.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|131|広告を広告する広告|reveal|なし|sheet4_10.png|移動経路・HitArea|画像不要または専用素材なし。意味一致するコード描画|移動する画像/対象をタップ|B|151件共通Widgetテスト対象|
|132|詳しく見る→詳細はありません|tapChallenge|sheet1_14.png|sheet5_17.png|HitArea・進捗|sheet1_14.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|133|何の商品でしょう？|reveal|なし|sheet5_15.png|移動経路・HitArea|画像不要または専用素材なし。意味一致するコード描画|移動する画像/対象をタップ|B|151件共通Widgetテスト対象|
|134|とにかく光る広告|reveal|sheet2_19.png|sheet4_10.png|移動経路・HitArea|sheet2_19.png|移動する画像/対象をタップ|A|151件共通Widgetテスト対象|
|135|とにかく揺れる広告|reveal|sheet2_20.png|sheet5_17.png|移動経路・HitArea|sheet2_20.png|移動する画像/対象をタップ|A|151件共通Widgetテスト対象|
|136|広告です！！！！！！|reveal|sheet1_17.png|sheet5_15.png|移動経路・HitArea|sheet1_17.png|移動する画像/対象をタップ|A|151件共通Widgetテスト対象|
|137|この広告には意味があります|reveal|なし|sheet4_10.png|移動経路・HitArea|画像不要または専用素材なし。意味一致するコード描画|移動する画像/対象をタップ|B|151件共通Widgetテスト対象|
|138|広告を見るための広告|reveal|なし|sheet5_17.png|移動経路・HitArea|画像不要または専用素材なし。意味一致するコード描画|移動する画像/対象をタップ|B|151件共通Widgetテスト対象|
|139|何も売らないセール|reveal|なし|sheet5_15.png|移動経路・HitArea|画像不要または専用素材なし。意味一致するコード描画|移動する画像/対象をタップ|B|151件共通Widgetテスト対象|
|140|クリックする場所がありません|reveal|なし|sheet4_10.png|移動経路・HitArea|画像不要または専用素材なし。意味一致するコード描画|移動する画像/対象をタップ|B|151件共通Widgetテスト対象|
|141|伝説の閉じるボタン|tapChallenge|sheet1_14.png|sheet4_09.png|HitArea・進捗|sheet1_14.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|142|広告王の王冠|tapChallenge|sheet1_02.png|sheet5_04.png|HitArea・進捗|sheet1_02.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|143|0.01％OFF祭|tapChallenge|sheet1_16.png|sheet5_20.png|HitArea・進捗|sheet1_16.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|144|幻の残り1個|tapChallenge|sheet2_17.png|sheet4_09.png|HitArea・進捗|sheet2_17.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|145|SSRより上っぽい何か|tapChallenge|sheet1_07.png|sheet5_04.png|HitArea・進捗|sheet1_07.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|146|広告博士・最終講義|choice|sheet2_05.png|sheet5_20.png|選択枠・正誤判定|sheet2_05.png|画像付き選択肢をタップ|A|151件共通Widgetテスト対象|
|147|王様、ついに助かる|pinPull|sheet1_02.png|sheet4_09.png|ピン・液体・順序判定|sheet1_02.png|ピンを順番にタップ|A|151件共通Widgetテスト対象|
|148|究極の広告パック|packOpen|sheet2_15.png|sheet5_04.png|スワイプ距離・開封枠|sheet2_15.png|画像付きパックを上へスワイプ|A|151件共通Widgetテスト対象|
|149|最後から2番目っぽい広告|reveal|なし|sheet5_20.png|移動経路・HitArea|画像不要または専用素材なし。意味一致するコード描画|移動する画像/対象をタップ|B|151件共通Widgetテスト対象|
|150|広告図鑑、ほぼ完成！|tapChallenge|sheet1_10.png|sheet4_09.png|HitArea・進捗|sheet1_10.png|画像/対象を3回タップ|A|151件共通Widgetテスト対象|
|151|幻の広告 ― アドゴン|finale|sheet1_10.png|sheet5_12.png|紋章・進捗・解放判定|sheet1_10.png|王冠を3回タップ|A|151件共通Widgetテスト対象|

## 評価集計

- A: 137件
- B: 14件
- C: 0件
- D: 0件

Aは意味一致する前景画像を操作対象または反応キャラクターとして統合済み。Bは背景を統合し、単純図形または専用素材不足部分をコード描画した広告。
